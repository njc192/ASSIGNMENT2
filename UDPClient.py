...
time_start       Stores the time the last message was sent.
time_end         Stores the time the last message was received.
rtt_max          Stores maximum RTT. Updated each time a message is received
rtt_min          Stores minimum RTT. Updated each time a message is received
received_count   No. of reponses received. Updated immediately on received msg
...
timeout_interval Sets the initial timeout to 1 second
last_packet_lost Stores whether last request timed out. Used for Dev RTT
'''
rtt_max = 0
rtt_min = float("inf")
time_start = 0
...
avg = float(total_time) / float(received_count)

# Display final calculations
print("Min RTT:         " + str(rtt_min) + " ms")
print("Max RTT:         " + str(rtt_max) + " ms")
print("Avg RTT:         " + str(avg) + " ms")
print("Packet Loss:     " + str(float(sent_count - received_count)/sent_count * 100.0) + "%");
print("Estimated RTT:   " + str(estimated_rtt) + " ms")
